import asyncio
from typing import List, Iterable, Awaitable, Coroutine, Any
...
async def get_users(uuids: List[int]) -> List[Coroutine[Any, Any, dict]]:
    return [async_get_user_from_fake_db(uuid) for uuid in uuids]


@async_timed()
async def main(users: Coroutine[Any, Any, List[Coroutine[Any, Any, dict]]]):
    result = []
